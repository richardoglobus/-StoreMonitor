          ...
          <Dialog open={isDialogOpen} onOpenChange={(v) => { setIsDialogOpen(v); if (!v) resetForm(); }}>
            ...
            <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 py-2">
                ...
                {header.purchaseOrderId && <div className="space-y-1 col-span-2"><Label>Commodity to receive in this GRN</Label><Select value={header.purchaseOrderLineId} onValueChange={v => { const po: any = (purchaseOrders || []).find((p: any) => String(p.id) === header.purchaseOrderId); const line: any = po?.lines?.[Number(v)]; if (line) { setHeader(h => ({ ...h, purchaseOrderLineId: v })); setLines([{ ...emptyLine(), itemId: line.itemId ? String(line.itemId) : "", itemCode: line.itemId ? String(line.itemId) : "", description: line.description || "", unit: line.unit || "", orderedQuantity: String(line.quantity || ""), qtyReceived: "", unitCost: String(line.unitPrice || "") }]); } }}><SelectTrigger><SelectValue placeholder="Select one PO commodity"/></SelectTrigger><SelectContent>{(((purchaseOrders || []).find((p: any) => String(p.id) === header.purchaseOrderId)?.lines) || []).map((line: any, index: number) => <SelectItem key={index} value={String(index)}>{line.description} — ordered {line.quantity} {line.unit}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted-foreground">Each commodity can be received in its own GRN. Enter the delivered quantity; the original ordered quantity remains unchanged.</p></div>}
              </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Commodity / Item</TableHead><TableHead>Qty ordered</TableHead><TableHead>Qty received</TableHead>{folder === "pending" && <TableHead>Balance</TableHead>}<TableHead>Approved / Voided by</TableHead><TableHead>Total (KES)</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading && Array.from({ length: 4 }).map((_, i) => <TableRow key={i}><TableCell colSpan={folder === "pending" ? 11 : 10}><Skeleton className="h-6 w-full"/></TableCell></TableRow>)}
            {!isLoading && visibleGrns.length === 0 && <TableRow><TableCell colSpan={folder === "pending" ? 11 : 10} className="text-center text-muted-foreground py-8">No {folder} GRNs.</TableCell></TableRow>}
            {visibleGrns.map(g => (
              <TableRow key={g.id}>
