...
export default function CatalogCategoriesPage() {
  ...
  return <Layout>
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <Card><CardHeader><CardTitle className="text-base">Imported Charge-Code Catalogue ({chargeCodes?.length || 0})</CardTitle><p className="text-sm text-muted-foreground">These are the 130 charge-code records imported from Category.xlsx. Select one when creating a catalog category.</p></CardHeader><CardContent className="p-0"><div className="max-h-96 overflow-y-auto"><Table><TableHeader><TableRow><TableHead className="w-36">Code</TableHead><TableHead>Description</TableHead></TableRow></TableHeader><TableBody>{(chargeCodes || []).map(code=><TableRow key={code.code}><TableCell className="font-mono font-semibold">{code.code}</TableCell><TableCell>{code.name}</TableCell></TableRow>)}{!chargeCodes?.length && <TableRow><TableCell colSpan={2} className="text-center py-8 text-muted-foreground">No charge codes loaded. Confirm the backend has been updated and restarted.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card>
