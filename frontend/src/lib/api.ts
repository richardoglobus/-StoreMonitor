...
export type GrnItem = { itemId?: number | null; itemCode: string | null; description: string; unit: string | null; qtyReceived: number; unitCost: number; totalCost: number; batchNo: string | null; expiryDate: string | null; chargeItemCode: string | null; folioNo: string | null };
export type Grn = { id: number; grnNo: string; date: string; receivedDate?: string | null; lpoNo: string | null; orderRefType?: string | null; orderRefNo?: string | null; supplierId: number; invoiceNo: string | null; items: GrnItem[]; totalAmount: number; status: "pending" | "approved" | "voided"; createdBy: number | null; createdAt: string; approvedBy: number | null; approvedAt: string | null; sourcePurchaseId?: number | null; sourcePurchaseOrderId?: number | null; sourcePurchaseOrderLineId?: number | null; orderedQuantity?: number | null; voidedBy?: number | null; voidedAt?: string | null; voidReason?: string | null; voidRequestStatus?: "pending" | "approved" | "rejected" | null; voidRequestedBy?: number | null; voidRequestedAt?: string | null; voidRequestedByName?: string | null; voidRequestReason?: string | null; voidReviewedBy?: number | null; voidReviewedByName?: string | null; voidReviewedAt?: string | null; voidReviewNote?: string | null; supplier?: Supplier | null };
...
export type PurchaseOrder = {
  ...
  taxEnabled?: boolean; taxPercent: number; lines: PurchaseOrderLine[];
...
export function useCreateJournalEntry(options?: MutOpts<JournalEntry, { data: any }>) { return useMutation({ mutationFn: ({ data }) => apiFetch<JournalEntry>("/api/accounts/journal-entries", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }), ...options?.mutation }); }
...
export function useGeneratePurchaseOrderGrns(options?: MutOpts<any[], { orderId: number }>) { return useMutation({ mutationFn: ({ orderId }) => apiFetch<any[]>(`/api/accounts/purchase-orders/${orderId}/generate-grns`, { method: "POST" }), ...options?.mutation }); }
