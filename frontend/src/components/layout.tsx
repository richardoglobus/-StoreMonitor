...
export function Layout({ children }: { children: ReactNode }) {
  ...
  const filteredNavItems = NAV_ITEMS.filter(item => user?.permissions?.[item.permission as keyof typeof user.permissions]);
  ...
  const NavLinks = () => (
    <>
      {filteredNavItems.map((item) => {
        ...
        return (
          <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${isActive ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      <header className="md:hidden flex items-center justify-between px-4 py-3 border-b bg-card">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-lg tracking-tight">Mukurweini Hospital Stores App</span>
        </div>
        <Sheet>
      ...
      <aside className="hidden md:flex flex-col w-64 border-r bg-card h-screen sticky top-0">
        <div className="p-6 flex items-center gap-2">
          ...
          <span className="font-bold text-xl tracking-tight text-foreground">Mukurweini Hospital Stores App</span>
        </div>
